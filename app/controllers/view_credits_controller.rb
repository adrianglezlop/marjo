class ViewCreditsController < ApplicationController
  ...
  def getArreglo
    ...
    @credit.product.numero_de_pagos_a_realizar.times do |n|
      ...
      @arreglo.push([ "#{n+1}",fecha.to_date.strftime("%d-%m-%Y"),"#{Dinero.to_money((@total-((n)*@pago)).round(2))}","#{Dinero.to_money(@capital.round(2))}","#{Dinero.to_money(@interes.round(2))}","#{Dinero.to_money(@pago.round(2))}","#{Dinero.to_money((@pago).round(2))}"])
