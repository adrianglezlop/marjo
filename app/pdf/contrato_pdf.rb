...
",:size =>10,:align => :justify 
    ...
    * El tabulador que se despliega en el presente punto hace referencia a que el abono está condicionado a que por cada $1,000.00 de apoyo a 'El beneficiario', deberá darse un abono por la cantidad de #{Dinero.to_money((1000*(1+@credit.product.taza_de_interes_ordinaria/100))/@credit.product.numero_de_pagos_a_realizar)} #{@credit.product.payout.periocidad}. El beneficio que adquirirá "La Fundacion" es del orden de #{'%.2f' % @credit.product.taza_de_interes_ordinaria} % #{@credit.product.plazo}.",:size =>10,:align => :justify 
    ...
    text" 'La Fundación' tiene en todo momento el derecho de negar cualquier pedido a 'El Beneficiario' cuando algún recibo se encuentre con más de 1 días de atraso, independientemente de que su apoyo sea menor de $ #{Dinero.to_money(@credit.monto_solicitud)} " ,:size =>10,:align => :justify 
    ...
    text" CUARTA: A falta de pago puntual de cualquier recibo emitido por 'La Fundación' de conformidad con las segunda y tercera, se dará por vencido este contrato, y en consecuencia se suspenderá la linea el apoyo económico, debiendo 'La Fundación' exigir a 'El Beneficiario'",:size =>10,:align => :justify 
