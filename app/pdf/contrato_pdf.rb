...
",:size =>10,:align => :justify 
    ...
    * El tabulador que se despliega en el presente punto hace referencia a que el abono está condicionado a que por cada $1,000.00 de apoyo al cliente, deberá darse un abono por la cantidad de #{Dinero.to_money((1000*(1+@credit.product.taza_de_interes_ordinaria/100))/@credit.product.numero_de_pagos_a_realizar)} #{@credit.product.payout.periocidad}. La tasa de interes del apoyo es del #{'%.2f' % @credit.product.taza_de_interes_ordinaria} % #{@credit.product.plazo}.",:size =>10,:align => :justify 
    ...
    text" 'La Fundación' tiene en todo momento el derecho de negar cualquier pedido a 'El Beneficiario' cuando algún recibo se encuentre con más de 1 días, independientemente de su adeudo será menor de $ #{Dinero.to_money(@credit.monto_solicitud)} " ,:size =>10,:align => :justify 
    ...
    text" La línea de apoyo otorgada jamás podrá exceder a la cantidad que resulte de 2 meses de sueldo que el cliente demuestre para la apertura del presente contrato de apertura de apoyo.",:size =>10,:align => :justify 
